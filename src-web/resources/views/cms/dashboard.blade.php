...
@section('content')
    ...
    @include('cms.partials.access-log-section', [
        'logs' => $logs,
        'logFilters' => $logFilters,
        'qrTypeLabels' => $qrTypeLabels,
    ])
@endsection
...
@section('scripts')
    ...
    <script>
        ...
        async function fetchAccessLogSection(url) {
            const section = document.getElementById('access-log-section');
            if (!section) return;

            if (accessLogAbortController) {
                accessLogAbortController.abort();
            }

            accessLogAbortController = new AbortController();
            ...
            try {
                const response = await fetch(url, {
                    ...
                    signal: accessLogAbortController.signal,
                });

                if (!response.ok) {
                    throw new Error('Failed to load log section');
                }

                const data = await response.json();
                section.outerHTML = data.html;
                window.history.replaceState({}, '', url);
                const updatedSection = document.getElementById('access-log-section');
                if (updatedSection) {
                ...
                const newSection = document.getElementById('access-log-section');
                if (newSection) {
                    newSection.classList.remove('opacity-60', 'pointer-events-none');
                }
            }
        }

        document.addEventListener('submit', event => {
            const form = event.target.closest('#access-log-filter');
            if (!form) return;

            event.preventDefault();
            const url = `${form.action}?${new URLSearchParams(new FormData(form)).toString()}`;
            fetchAccessLogSection(url);
        });

        document.addEventListener('click', event => {
            const link = event.target.closest('#access-log-section a');
            if (!link) return;

            event.preventDefault();
            fetchAccessLogSection(link.href);
        });
    </script>
@endsection
